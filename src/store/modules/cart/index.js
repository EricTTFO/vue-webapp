...
import Vue from 'vue';
const isEveryRight = (arr, fn) => arr.map(fn).every(val => val);
const state = {
  cartInfo: {},
};
const mutations = {
  [TYPES.SET_CARTLIST](state, data) {
    state.cartInfo = data;
  },
  [TYPES.SET_SELECT_GOOD](state, { id, sellerId }) {
    ...
    shops[`shop_${sellerId}`].fields.checked = gorups[`shop_${sellerId}`]
      .map(id => goods[id].fields.checked)
      .every(val => val);
    // 检查是否全选
    footer.fields.checkAll.checked = isEveryRight(
      Object.keys(shops),
      id => (id = shops[id].fields.checked),
    );
  },
  [TYPES.SET_SELECT_SHOP](state, { id }) {
    const { goods, shops, gorups, footer } = state.cartInfo;
  ...
  [TYPES.SET_SELECT_CART_ALL](state) {
  ...
  [TYPES.SET_BUY_QUANTITY](state, { item, type }) {
    ...
    const fetchQuantity = (values, type) => {
  ...
  [TYPES.DEL_CART_GOOD](state, item) {
    ...
    gorups[shopkey] = gorups[shopkey].filter(val => val != goodkey);
